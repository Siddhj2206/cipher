# Structured error system (E001–E099)
...
| E003 | `ProfileNotFound { name }` | A named profile could not be resolved | 2 |
| E004 | `Glossary(String)` | Glossary JSON read/parse/write failure | 1 |
...
| E099 | `Other(#[from] anyhow::Error)` | Escape hatch for anything not yet classified | 70 |
...
- Exit code 70 (EX_SOFTWARE) is reserved for "internal" failures (state corruption, unclassified). Codes 1–4 follow the sysexits-style mapping for the relevant failure class.
- No `serde` derive on `Error` yet; the JSON envelope for error responses is tracked separately (issue #92).
- `anyhow` remains a dependency solely for the `E099` bridge; production code no longer imports it.
